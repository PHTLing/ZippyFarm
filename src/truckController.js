...
export class TruckController {
  ...
  handleMovement(keyboardState) {
    ...
    if (keyboardState["ArrowUp"] || keyboardState["KeyW"]) {
      ...
      const reversePower = 0.8; // Giảm tốc độ lùi
      ...
      moveSpeed = Math.sqrt(
        currentLinVel.x * currentLinVel.x + currentLinVel.z * currentLinVel.z
      );
      // Giữ dấu của tốc độ để biết đang tiến hay lùi
      const dotForward = forwardVector.dot(
        new THREE.Vector3(currentLinVel.x, 0, currentLinVel.z)
      );
    ...
    if (
      (keyboardState["ArrowUp"] && keyboardState["ArrowLeft"]) ||
      (keyboardState["KeyW"] && keyboardState["KeyA"])
    ) {
      angVelY = this.truckRotationSpeed * boost * stop;
    } else if (
      (keyboardState["ArrowUp"] && keyboardState["ArrowRight"]) ||
      (keyboardState["KeyW"] && keyboardState["KeyD"])
    ) {
      angVelY = -this.truckRotationSpeed * boost * stop;
    }

    if (
      (keyboardState["ArrowDown"] && keyboardState["ArrowLeft"]) ||
      (keyboardState["KeyS"] && keyboardState["KeyA"])
    ) {
      angVelY = -this.truckRotationSpeed * boost * stop;
    } else if (
      (keyboardState["ArrowDown"] && keyboardState["ArrowRight"]) ||
      (keyboardState["KeyS"] && keyboardState["KeyD"])
    ) {
      angVelY = this.truckRotationSpeed * boost * stop;
    }

    // Đặt vận tốc tuyến tính, giữ nguyên vận tốc trục Y để trọng lực hoạt động
    this.truckRigidBody.setLinvel(
      { x: newLinVelX, y: currentLinVel.y, z: newLinVelZ },
      true
    );
    ...
    this.truckRigidBody.setRotation(
      new this.physicsManager.RAPIER.Quaternion(
    ...
    const wheelRotationAmount =
      ((moveSpeed * this.wheelRotationFactor) / (2 * Math.PI)) * 0.1; // 0.1 là delta time nhỏ
    ...
    const currentSteerAngleL = this.frontWheelLGroup
      ? this.frontWheelLGroup.rotation.y
      : 0;
    const currentSteerAngleR = this.frontWheelRGroup
      ? this.frontWheelRGroup.rotation.y
      : 0;

    const lerpFactor = 0.1; // Tùy chỉnh độ mượt khi bẻ lái

    if (this.frontWheelLGroup) {
      this.frontWheelLGroup.rotation.y = THREE.MathUtils.lerp(
        currentSteerAngleL,
        targetSteerAngle,
        lerpFactor
      );
    ...
    if (this.frontWheelRGroup) {
      this.frontWheelRGroup.rotation.y = THREE.MathUtils.lerp(
        currentSteerAngleR,
        targetSteerAngle,
        lerpFactor
      );
    ...
    // THÊM: Áp dụng lực ép xuống (downforce) để xe bám đường hơn
    const downForceVector = { x: 0.0, y: -this.downforce, z: 0.0 };
    this.truckRigidBody.addForce(downForceVector, true);
    // =====================================
  }
  ...
  resetMovement() {
    if (
      this.truckRigidBody &&
      this.physicsManager &&
      this.physicsManager.RAPIER
    ) {
      this.truckRigidBody.setLinvel(
        new this.physicsManager.RAPIER.Vector3(0, 0, 0),
        true
      );
      this.truckRigidBody.setAngvel(
        new this.physicsManager.RAPIER.Vector3(0, 0, 0),
        true
      );
    }
