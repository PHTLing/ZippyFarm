...
export class PhysicsManager {
    constructor(RAPIER_LIB, soundManager) {
        ...
        this.truckInfo = { mesh: null, rigidBody: null, collider: null, debugMesh: null }; // Thông tin xe tải
    ...
    createBox(options) {
        const { position, quaternion, halfExtents, isDynamic, friction, restitution, linearDamping, angularDamping, debugColor } = options;

        const rigidBodyDesc = isDynamic ? this.RAPIER.RigidBodyDesc.dynamic() : this.RAPIER.RigidBodyDesc.fixed();
        if (linearDamping !== undefined) rigidBodyDesc.setLinearDamping(linearDamping);
        if (angularDamping !== undefined) rigidBodyDesc.setAngularDamping(angularDamping);

        rigidBodyDesc.setTranslation(position.x, position.y, position.z);
        if (quaternion) {
            rigidBodyDesc.setRotation(new this.RAPIER.Quaternion(quaternion.x, quaternion.y, quaternion.z, quaternion.w));
        }
        ...
        if (options.mesh) { // Đảm bảo options.mesh tồn tại trước khi push
    ...
    createCylinder(options) {
        const { position, radiusTop, radiusBottom, height, radialSegments, isDynamic, friction, restitution, debugColor } = options;

        const rigidBodyDesc = isDynamic ? this.RAPIER.RigidBodyDesc.dynamic() : this.RAPIER.RigidBodyDesc.fixed();
        rigidBodyDesc.setTranslation(position.x, position.y, position.z);
        const rigidBody = this.world.createRigidBody(rigidBodyDesc);

        const colliderDesc = this.RAPIER.ColliderDesc.cylinder(height / 2, radiusTop)
            .setFriction(friction)
            .setRestitution(restitution)
            .setActiveEvents(this.RAPIER.ActiveEvents.COLLISION_EVENTS);
        const collider = this.world.createCollider(colliderDesc, rigidBody);
    ...
    createSphere(options) {
        const { position, radius, isDynamic, friction, restitution, debugColor } = options;
    ...
    createTrimesh(options) {
        const { meshes, position, quaternion, friction, restitution, debugColor, isDynamic } = options;
        ...
        collidersForCompound.forEach(cDesc => this.world.createCollider(cDesc, rigidBody));
    ...
    handleCollisionEvents() {
        this.eventQueue.drainCollisionEvents((handle1, handle2, started) => {
            ...
            const truckColliderHandle = this.truckInfo.collider ? this.truckInfo.collider.handle : null;
            ...
            if (truckIsColliding && otherColliderHandle !== null) {
                ...
                const isTestObject = this.testObjects.some(obj => obj.rigidBody === otherRigidBody);
                ...
                if (objectInfo) {
                    ...
                    if (objectInfo.type === 'fallable' && otherRigidBody.bodyType() === this.RAPIER.RigidBodyType.Fixed) {
                ...
                // Xử lý va chạm với các đối tượng test (hộp, trụ, cầu)
                    ...
                    if (otherCollider.shapeType === this.RAPIER.ShapeType.Ball) { // Kiểm tra loại hình dạng là Ball
                        console.log(`VA CHẠM: Xe tải đâm vào đối tượng test là CẦU.`);
                        const truckLinvel = this.truckInfo.rigidBody.linvel();
                        const impactForce = new this.RAPIER.Vector3(truckLinvel.x * 0.1, 0, truckLinvel.z * 0.1); // Lực nhỏ dựa trên vận tốc xe tải
    ...
    updateThreeJSObjects() {
        ...
        if (this.truckInfo.mesh && this.truckInfo.rigidBody) {
            ...
            this.truckInfo.mesh.quaternion.set(rotation.x, rotation.y, rotation.z, rotation.w);
            if (this.truckInfo.debugMesh) {
                this.truckInfo.debugMesh.position.copy(this.truckInfo.mesh.position);
                this.truckInfo.debugMesh.quaternion.copy(this.truckInfo.mesh.quaternion);
            }
        }
        ...
        if (this.gltfObjectsMap) {
            this.gltfObjectsMap.forEach(item => {
                ...
                if (item.rigidBody.bodyType() === this.RAPIER.RigidBodyType.Dynamic) {
                    ...
                    this.tempVector3.set(rigidBodyWorldPosition.x, rigidBodyWorldPosition.y, rigidBodyWorldPosition.z);
