...
async function setupRapier() {
    ...
    const farmLoadPromise = new Promise((resolve, reject) => {
        loader.load('assets/Farm_N.glb', function (gltf) {
            ...
            farmScene.updateMatrixWorld(true); // Cập nhật ma trận thế giới để tính toán bounding box chính xác
            ...
            farmScene.children.forEach((parentObject) => {
                ...
                // Lấy vị trí và góc quay ban đầu của đối tượng Three.js trong không gian thế giới
                ...
                if (isTrimeshObject) {
                    ...
                    // Tạo rigid body cho đối tượng cha tại vị trí thế giới của nó
                    // Đối với Trimesh, rigid body cần được đặt tại vị trí thế giới mà các đỉnh Trimesh được định nghĩa
                    ...
                    // Lấy ma trận nghịch đảo của rigid body để chuyển đổi đỉnh từ world space về local space của rigid body
                    ...
                    meshesInGroup.forEach(meshChild => {
                        ...
                        const transformedLocalVertices = []; // Vertices local to rigid body
                        const meshChildWorldMatrix = meshChild.matrixWorld;

                        for (let i = 0; i < positions.length; i += 3) {
                            tempPos.set(positions[i], positions[i + 1], positions[i + 2]);
                            tempPos.applyMatrix4(meshChildWorldMatrix); // Chuyển đổi từ local của mesh con sang world space
                            tempPos.applyMatrix4(rigidBodyInverseWorldMatrix); // Chuyển đổi từ world space sang local space của rigid body
                    ...
                    // Debug mesh của Trimesh được đặt tại vị trí của rigid body
                    ...
                    gltfObjectsMap.set(rigidBody.handle, {
                        mesh: parentObject, // THREE.js Object3D
                        rigidBody: rigidBody,
                        debugMesh: debugMesh,
                        type: objectType
                    });
                    ...
                    const boundingBox = new THREE.Box3().setFromObject(parentObject, true); // Tính toán bounding box trong tọa độ thế giới
...
function handleTruckMovement() {
    ...
    if (isNaN(linVel.x) || isNaN(linVel.z) ||
        ...
        truckRigidBody.setRotation(new RAPIER.Quaternion(0, 0, 0, 1), true);
...
function animate() {
    ...
    cylinders.forEach(obj => {
        ...
        obj.mesh.position.set(position.x, obj.rigidBody.translation().y, obj.rigidBody.translation().z); // Fix typo, should be position.y, position.z
    ...
    gltfObjectsMap.forEach(item => {
        ...
        if (item.rigidBody.bodyType() === RAPIER.RigidBodyType.Dynamic) {
            ...
            // Lấy ma trận nghịch đảo của parent của mesh (farmScene) để chuyển đổi từ world sang local
            const parent = item.mesh.parent;
            parent.updateMatrixWorld(true); // Đảm bảo ma trận thế giới của parent được cập nhật
            
            // Sử dụng tempMatrix4.copy().invert() để đảm bảo getInverse được gọi đúng cách
            tempMatrix4.copy(parent.matrixWorld).invert();
            
            // Áp dụng ma trận nghịch đảo cho vị trí thế giới của rigid body để có vị trí local
            ...
            // Tính góc quay local: inv(parent_world_quat) * rigidBody_world_quat
            parent.getWorldQuaternion(tempQuaternion); // Lấy góc quay thế giới của parent
            tempQuaternion.invert(); // Nghịch đảo của góc quay parent
            tempQuaternion.multiply(rigidBodyWorldQuaternion); // Nhân với góc quay thế giới của rigid body
