...
export class SoundManager {
  ...
  async loadSounds() {
    ...
    try {
      ...
      this.backgroundMusic.setVolume(0.2); // Sửa lại âm lượng gốc
    } catch (error) {
  ...
  // Các hàm điều khiển âm thanh riêng biệt (Giữ nguyên)
  playBackgroundMusic() {
  ...
  updateEngineVolumeAndPitch(speed, maxSpeed, volumeBoost = 1) {
    if (this.engineSound) {
      ...
      let baseVolume = normalizedSpeed * 0.1;
      this.engineSound.setVolume(baseVolume * volumeBoost);
      this.engineSound.setPlaybackRate(0.8 + normalizedSpeed * 0.4);
    }
  }

  // --- CHỈ THÊM HÀM NÀY ---
  // Dừng tất cả các âm thanh đang phát
  stopAllSounds() {
    this.stopBackgroundMusic();
    this.stopEngineSound();
    this.stopHornPressSound();
    // Các âm thanh không lặp lại như brake, click, collision không cần gọi stop ở đây
    // vì chúng sẽ tự dừng.
  }
}
