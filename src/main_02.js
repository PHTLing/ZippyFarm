...
async function setupRapier() {
    ...
    const truckLoadPromise = new Promise((resolve, reject) => {
        loader.load('assets/Truck.glb', function (gltf) {
            ...
            truckMesh.scale.set(0.5, 0.5, 0.5); // Đảm bảo kích thước xe không quá nhỏ
            ...
            // Tạo RigidBody cho xe tải
            const truckRigidBodyDesc = RAPIER.RigidBodyDesc.kinematicPositionBased()
            ...
            // Collider cho xe tải (Box)
            const truckHalfExtents = new RAPIER.Vector3(0.5 * 0.5, 0.5 * 0.5, 1 * 0.5);
            const truckColliderDesc = RAPIER.ColliderDesc.cuboid(truckHalfExtents.x, truckHalfExtents.y, truckHalfExtents.z);
            world.createCollider(truckColliderDesc, truckRigidBody);
...
function handleTruckMovement() {
    ...
    // Kiểm tra loại RigidBody để áp dụng chuyển động
    if (truckRigidBody.isKinematic()) {
        const currentPos = truckRigidBody.translation();

        // Tính toán vị trí mới
        const deltaPos = new THREE.Vector3(linVel.x, linVel.y, linVel.z).multiplyScalar(timeStep);
        const newPos = new RAPIER.Vector3(currentPos.x + deltaPos.x, currentPos.y + deltaPos.y, currentPos.z + deltaPos.z);

        // Tính toán góc quay mới
        const deltaRotation = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), angVelY * timeStep);
        const newRotation = new THREE.Quaternion().copy(currentQuaternion).multiply(deltaRotation).normalize(); // Dùng currentQuaternion từ Three.js

        // --- Debugging logs ---
        console.log("--- Truck Movement Debug ---");
        console.log("Current Truck Pos (Rapier):", currentPos.x.toFixed(2), currentPos.y.toFixed(2), currentPos.z.toFixed(2));
        ...
        if (isNaN(linVel.x) || isNaN(linVel.z) || isNaN(newPos.x) || isNaN(newPos.z) ||
            isNaN(currentRotation.x) || isNaN(currentRotation.y) || isNaN(currentRotation.z) || isNaN(currentRotation.w)) {
            console.error("Lỗi: Giá trị NaN được phát hiện trong vận tốc, vị trí, hoặc rotation của xe tải!");
            ...
            truckRigidBody.setNextKinematicRotation(new RAPIER.Quaternion(0, 0, 0, 1));
            // Cũng có thể đặt lại vị trí nếu nó cũng bị ảnh hưởng
            // truckRigidBody.setNextKinematicTranslation(new RAPIER.Vector3(-5, 0.5, -5));
        }
        console.log("New Truck Pos (calculated):", newPos.x.toFixed(2), newPos.y.toFixed(2), newPos.z.toFixed(2));
        // --- End Debugging logs ---

        truckRigidBody.setNextKinematicTranslation(newPos);
        truckRigidBody.setNextKinematicRotation({x: newRotation.x, y: newRotation.y, z: newRotation.z, w: newRotation.w});

    } else { // Nếu là dynamic body (nếu bạn thay đổi loại body sau này)
        truckRigidBody.setLinvel(linVel, true);
...
function animate() {
    ...
    if (!controls.enabled) { // Nếu OrbitControls đang tắt, tức là bạn muốn điều khiển xe
        ...
        if (truckRigidBody && truckRigidBody.isKinematic()) {
            truckRigidBody.setNextKinematicTranslation(truckRigidBody.translation()); // Giữ nguyên vị trí
            truckRigidBody.setNextKinematicRotation(truckRigidBody.rotation()); // Giữ nguyên góc quay
        } else if (truckRigidBody) {
             truckRigidBody.setLinvel({x:0, y:0, z:0}, true);
             truckRigidBody.setAngvel({x:0, y:0, z:0}, true);
        }
    }
    ...
    // Đồng bộ vị trí và góc quay của các debug meshes
